...
function Validator(formSelector) {
    ...
    var validatorRules = {
        required(value) {
            ...
            return value ? '' : `Veuillez saisir ceci !`;
        },
        email(value) {
            var regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
            return regex.test(value) ? '' : `L'email saisi n'est pas valide !`;
        },
        min(min) {
            return (value) => {
                return value.length >= min ? '' : `Veuillez entrer un minimum${min} caracteres!`;
            }
        },
        max(max) {
            return (value) => {
                return value.length <= max ? '' : `Veuillez entrer un maximum ${max} caracteres!`;
            }
        },
        confirm(selector) {
            return (value) => {
                return value == document.querySelector(selector).value ? '' : `Le mot de passe saisi à nouveau est incorrect`;
            }
        },
        checkone(name) {
            return () => {
                var checked = formElement.querySelector(`[name="${name}"]:checked`);
                return checked ? '' : `Veuillez sélectionner au moins 1 option`;
            }
        }
